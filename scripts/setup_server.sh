...
if ! "$CONDA_BIN" env list | awk '{print $1}' | grep -Fxq "$CONDA_ENV_NAME"; then
  "$CONDA_BIN" create -n "$CONDA_ENV_NAME" python=3.11 -y
fi

"$CONDA_BIN" run -n "$CONDA_ENV_NAME" python -m pip install --upgrade pip
"$CONDA_BIN" run -n "$CONDA_ENV_NAME" python -m pip install -r requirements.txt

echo "Server conda environment is ready: $CONDA_ENV_NAME"
