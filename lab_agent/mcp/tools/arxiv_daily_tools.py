...
from ...utils import project_path_str, today_str, yesterday_str
...
class ArxivDailyTools(BaseTool):
    ...
    async def _generate_daily_report(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"馃殌 MCP: Starting daily report generation for {parsed_date} (force={force})")
        
        try:
            # Check if report already exists (unless forced)
            if not force:
                self.logger.info(f"馃搵 MCP: Checking if report exists for {parsed_date}")
                existing_result = self.arxiv_agent._get_report(parsed_date)
                if existing_result['success']:
                    self.logger.info(f"馃搵 MCP: Report already exists for {parsed_date}")
                    return self.format_success_response(
            ...
            self.logger.info(f"馃摑 MCP: Generating new report with task: {task}")
            result = await self.arxiv_agent.process_task(task)
            self.logger.info(f"馃搳 MCP: Report generation result: {result.get('success', False)} - {result.get('message', 'No message')}")
    ...
    async def _search_papers_by_author(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"馃攳 MCP: Searching for author '{author_name}' in report for {parsed_date} (match_type: {match_type})")
